...
function win(userChoice, computerChoice) {
  ...
  const cpuFontSize = "Cpu".fontsize(3).sub();
  result_div.innerHTML =
    converToWords(userChoice) +
    userFontSize +
    " Beats " +
    converToWords(computerChoice) +
    cpuFontSize +
    " You wins.." +
    "  &#128165";
  document.getElementById(userChoice).classList.add("green_glue");
  setTimeout(function () {
    document.getElementById(userChoice).classList.remove("green_glue");
  }, 1200);
}
...
function lose(userChoice, computerChoice) {
  ...
  setTimeout(function () {
    document.getElementById(userChoice).classList.remove("red_glue");
  }, 1200);
}
